<template>
  <div :class="position" class="flex items-center">
    <!-- valid not loading -->
    <button v-if="props.isValidState && !props.submitLoading" @click="$emit('submit')" type="button"
      :class="[bttnSize, colorClasses, width]"
      class="inline-flex items-center justify-center border border-transparent font-bold rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-offset-1 transition-hover">
      <div v-if="props.svg" class="mr-3" v-html="props.svg"></div> {{ props.submitText }} </button>
    <!-- not valid not loading -->
    <button v-if="!props.isValidState && !props.submitLoading" type="button" disabled
      :class="[bttnSize, colorClassesDisabled, width]"
      class="inline-flex items-center justify-center border border-transparent font-bold rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-offset-1 transition-hover">
    ...
    <!-- loading -->
    <button v-if="props.submitLoading" type="button" :class="[colorClassesDisabled, bttnSize, width]"
      class="inline-flex items-center justify-center justify-center border border-transparent font-bold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 cursor-default"
      disabled>
